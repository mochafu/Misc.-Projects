...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        while (location and not location.is_hive):
            if location.bees and steps >= self.lower_bound and steps <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount: float):
        ...
        if self.place is None:
            additional =  self.damage
        for bee in list(temp.bees):
            bee.reduce_health(amount+additional)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.cooldown:
            ...
            temp = random_bee(self.place.bees)
    ...
    def action(self, gamestate: GameState):
        ...
        while place is not None:
            ...
            if ant is not None:
                if ant.is_container:
                    ant.double()
                    if ant.ant_contained is not None:   # <-- guard
